...
(function($){
	...
	SITE_SS.styleSwitcher = function(options){
		this.options = options;
	};

	ss = SITE_SS.styleSwitcher.prototype;
	...
	ss.buildSwitcher = function(){
		var self = this,
			...
			opened = sessionStorage.site_ss_opened === undefined ? self.options.switcher_opened :
				JSON.parse(sessionStorage.site_ss_opened),
			$blocks;

		self.$el = $('<div />', {'class':'site-style-switcher'})
			.prependTo($parent);
	...
	ss.toggleSwitcher = function(open, animate){
		var self = this,
			...
			doOnComplete = function(){
				...
				self.opened = open;
				sessionStorage.site_ss_opened = open;
			};
	...
	ss.addColorOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new SITE_SS.colorSetter(id, color, self.$el, self.blockSize).init();
			}
		}
	};	
	...
	ss.addPatternOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new SITE_SS.patternSetter(id, pattern, self.$el, self.blockSize).init();
			}
		}
	};
	...
	ss.addSelectOptions = function(){
		...
		if(len){
			for(i=0; i<len; i++){
				...
				new SITE_SS.selectSetter(id, select, $parent).init();
			}
		}
	};	
	...
	SITE_SS.patternSetter = function(id, options, $parent, size){
		this.itemId = id;
		this.options = options;
		this.$parent = $parent;
		this.elSize = size;
	};
	ps = SITE_SS.patternSetter.prototype;
	...
	SITE_SS.colorSetter = function(id, options, $parent, size){
		this.itemId = id;
		this.options = options;
		this.$parent = $parent;
		this.elSize = size;
	...
	cs = SITE_SS.colorSetter.prototype;
	...
	SITE_SS.selectSetter = function(id, options, $parent){
		this.itemId = id;
	...
	var ssel = SITE_SS.selectSetter.prototype;
