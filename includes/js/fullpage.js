...
"use strict";
	 ...
	 */
	SITE.Fullpage = function(selector, options){
		// return;
		var defaults = {
	...
	SITE.Fullpage.prototype = {
		...
		init : function(){
			...
			if(this.secNum){
				...
				this.$win = $(window);
				this.slides = [];
				this.mobile = SITE.utils.checkIfMobile();
		...
		loadSlider : function(){
			...
			this.loader = new SITE.Loader(this.$sections, {bgElements : '.slide'});
		...
		doOnLeave : function(index, nextIndex, direction){
			...
			if($video.length && SITE.supportsVideo()){
				$video.get(0).pause();
			}
	...
	SITE.Loader = function($elements, options){
		var defaults = {
			loadImg : true,
			loadBg : true,
			onAllLoaded : null
	...
	SITE.Loader.prototype = {
		...
		_loadElement : function(index){
			...
			$(imagesToLoad).siteOnImgLoaded({callback:function(){
				// setTimeout(function(){
					imagesLoaded = true;
